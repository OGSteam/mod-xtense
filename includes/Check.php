...
abstract class Check {
	...
	static function planet_name($string) {
		//return preg_match('![A-Z0-9ÃƒÂ©ÃƒÂ¨_. -]{1,20}!i', $string);
		return preg_match('!.{1,20}!i', $string);
	}
	
	static function player_status($string) {
		return preg_match('!^[AsnfdvbiIoph]*$!', $string);//fdvbiIoph en franÃƒÂ§ais, snvbiIoph in english
	}
	...
	static function coords($string, $exp = 0) {
		global $server_config;
		if ($string == "unknown") return true; //cas avec une seule planÃƒÂ¨te
